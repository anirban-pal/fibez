class fibnetwork {
  ...
  public:
    int np,nb,nf,nt,ncc,tstep,ncon,nfdof,minstep;
    double bounds[3][2];
    ...
    std::vector<Vector3> CPx,CPv,CPf,CPa,CPw;
    std::vector<cBezier> Bz_list;
    std::vector<std::vector<int>> fibers, CP_bid; 
    std::vector<int> CP_fid; 
    
    gsl_matrix *GM, *GMdot, *GN, *GMinv, *GM3, *GM3inv;
    gsl_matrix *Psys, *Qsys, *Rsys, *Fsys, *Wsys, *qH, *pH;
    gsl_matrix *Lsys, *Asys, *bsys, *qsys, *psys, *dqsys, *b2sys, *Hq, *MA, *AMA;
    
    gsl_matrix *MAX, *X, *AHq;
...
void fibnetwork::readfile(char *filename)
{
  ...
  loop(i,np) {
    sss ss;
    double x[3],w[3]; int tmp,fdof[3];
    std::getline(infile, line); ss << line; 
    ss >> tmp >> x[0] >> x[1] >> x[2] >> w[0] >> w[1] >> w[2] >> fdof[0] >> fdof[1] >> fdof[2];
    
    loop(j,3) if(fdof[j]==0) fixed_dof.insert(std::make_pair((tmp-1)*3+j+1, x[j] ));
    
    double rn = RTOL*(((double) std::rand() / (RAND_MAX))*2 - 1.0);
    
    CPx.push_back(Vector3(x[0]+rn,x[1]+rn,x[2]+rn));
    CPv.push_back(Vector3(0,0,0));
    CPw.push_back(Vector3(w[0],w[1],w[2]));
...
void fibnetwork::printlammps_cps(char *filename, char *mode)
{
  ...
  fprintf(fp, "ITEM: ATOMS id mol type q xu yu zu wx wy wz\n");
  
  loop(i,np) {
    fprintf(fp, "%d %d 1 0.00 %.6f %.6f %.6f %.6f %.6f %.6f\n", i+1, CP_fid[i], CPx[i].comp[0], CPx[i].comp[1], CPx[i].comp[2], CPw[i].comp[0], CPw[i].comp[1], CPw[i].comp[2]);
  }
...
double VdV(unsigned n, const double *input, double *grad, void *fn_data)
{
  ...
  loop(i,FN->nb) {
    ...
    loop(j,4) {
      ...
      loop(k,3) {
        ...
        FN->Bz_list[i].v[j*3+k] = 0.0;
        FN->Bz_list[i].w[j*3+k] = 0.0;
			}
    }
  ...
  FN->totE = FN->axialE + FN->bendE + FN->cohE;
  fprintf(FN->flog,"NL Minimization: %6d %14.8f %14.8f %14.8f %14.8f %14.8f\n", FN->minstep, FN->axialE, FN->bendE, FN->cohE, FN->kinE, FN->totE); 
  FN->minstep++;
...
void fibnetwork::minimize()
{
  ...
  if(1) {
    ...
    if (status < 0) {
      ...
      printlammps_cps((char*) DUMP1,(char*) "a");
      printlammps((char*) DUMP2,(char*) "a", NBEZ);  
  }
...
double fibnetwork::dMidQ(int l, int k, int m, int n)
{
    double res1 = 0;
    loop(i,np) loop(j,np) {
      res1 -= gsl_matrix_get(GMinv,l,i) * dMdQ(i,j,m,n) * gsl_matrix_get(GMinv,j,k);
    }
    
    return res1;
}

void fibnetwork::compute_dH()
{  
  assemble_matrices(); 
  
  gsl_matrix_set_zero(GM3);
  
  loop(i,np) loop(j,np) {
    double val = gsl_matrix_get(GM,i,j);
    gsl_matrix_set(GM3, 3*i,3*j,  val);
    gsl_matrix_set(GM3, 3*i+1,3*j+1, val );
    gsl_matrix_set(GM3, 3*i+2,3*j+2, val );
  } 
  
  //gsl_matrix_print(GM,"GM:");
  //gsl_matrix_print(GM3,"GM3:");
  
  gsl_matrix *GMtmp = gsl_matrix_alloc (np, np);
  gsl_matrix_memcpy(GMtmp, GM);
    
  gsl_permutation *p = gsl_permutation_alloc(np); int s;
  gsl_linalg_LU_decomp(GMtmp, p, &s); gsl_linalg_LU_invert(GMtmp, p, GMinv); gsl_permutation_free(p);
  
  gsl_matrix_free(GMtmp);
  
  //gsl_matrix_print(GMinv,"GMinv:");
  //gsl_matrix_print(Wsys,"Wsys:");
  
  gsl_matrix_set_zero(GM3inv);
  
  loop(i,np) loop(j,np) {
    double val = gsl_matrix_get(GMinv,i,j);
    gsl_matrix_set(GM3inv, 3*i,3*j,  val);
    gsl_matrix_set(GM3inv, 3*i+1,3*j+1, val );
    gsl_matrix_set(GM3inv, 3*i+2,3*j+2, val );
  } 
  
  //gsl_matrix_print(GMinv,"GMinv:");
  //gsl_matrix_print(GM3inv,"GM3inv:");
  
  {
    ...
    gsl_matrix_memcpy(GMtmp1, GM3);
    
    loop(i,np) loop(j,3) {
        gsl_vector_set(wsys, 3*i+j, gsl_matrix_get(Wsys,i,j));
    }
    ...
    loop(i,np) loop(j,3) {
        gsl_matrix_set(pH, i, j, gsl_vector_get(vsys,3*i+j));
    }
